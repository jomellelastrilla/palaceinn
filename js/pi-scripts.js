...
$(function () {
  ...
  function initializeMap(hotels) {
    ...
    hotels.forEach(function(hotel) {
      ...
      var customIcon = L.divIcon({
        ...
        html: '<div class="pin-icon" style="background-image:url(' + hotel.map_pin + ')">' + hotel.order + '</div>',              
        iconSize: [37, 56], // Size of the icon
        iconAnchor: [16, 32], // Anchor point of the icon, center bottom
        popupAnchor: [0, -32] // Popup anchor relative to the icon
      });
      ...
      var popupContent = '<div clas="pi-popup">' +
                        '<h3>Palace Inn ' + hotel.title + '</h3>' +
                        '<img src="' + hotel.image + '" alt="' + hotel.title + '" style="max-height: 200px;">' +
                        '<div class="pi-popup-contacts '+ hotel.color +'">' +
                        '<div class="pi-popup-item"><p>' + hotel.address + '</p>' +
                        '<p><strong>Phone:</strong> ' + hotel.phone + '</p></div><div class="pi-popup-item">' +
                        '<a class="cta" href="' + hotel.link + '" >More Info</a><a class="cta" href="' + hotel.booking_url + '" target="_blank">Book Now</a>' +
                        '</div></div>' +
                        '</div>';
      ...
      marker.on('click', function(e) {
        ...
        const offsetLatlng = map.layerPointToLatLng(map.latLngToLayerPoint(latlng).add(offsetPixels));
  ...
  function calculateZoomLevel(mapParams, latLng, radiusMiles) {
    ...
    try {
      ...
      const zoomLevel = mapParams.getBoundsZoom(latLngObject.toBounds(radiusMeters));
  ...
  if ($('#form-field-pi_hotel_lists').length) {
    ...
    const hotelIdFromUrl = new URLSearchParams(window.location.search).get('hotel_id');
    ...
    hotels.forEach(hotel => {
        ...
        option.val(hotel.ID).text(decodedTitle);
  ...
  if ($('#form-field-non_property_related-0')){
    $('#form-field-non_property_related-0').on('change', function() {
